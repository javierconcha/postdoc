...
The mean of all the diurnal SD is an indicator of the temporal stability of the selected homogeneous ocean region throughout the day. 
As mentioned before, the hope is to have minimal variation during the day over this region .
These mean diurnal SD values for the GCW region provides an approximate measure of the minimum $R_{rs}$ difference required to detect diurnal variability (\autoref{tab:diurnal_var}).

Also, the percent coefficient of variation (CV[\%]), defined as the SD divided by the mean, was calculated for each day.
Then, the statistics for the daily CV[\%] were calculated: minimum, maximum, mean, median and SD (\autoref{tab:diurnal_var}). The diurnal mean and SD were calculated only if at least three values per day were valid. This analysis was performed for all the data and for only summer, when the variability due to change in the water properties are minimal (\autoref{tab:diurnal_var})\todo{include Antonio's comment}\todo{talk about the results in the table}.
...
Second, in order to have a better estimation of the uncertainties, the relative difference of time of the day $t$ with respect to the 4h time of day ($R\Delta_t[\%]$) was calculated. 
...
\todo{update values from the table}\autoref{fig:Diff4th} and \autoref{tab:rel_diff} show the mean and the standard deviation (error bar in the figure) of the $R\Delta_t[\%]$ for each time of the day for the whole mission for $R_{rs}(\lambda)$ and Chlorophyll-{\it a}, POC and $a_g(412)$.
...
\begin{landscape}
  \begin{table}[htbp!]
  %\internallinenumbers
  \caption{The diurnal variability was quantized by calculating the diurnal mean and standard deviation (SD) for each day. Then, two times the mean of diurnal SD is considered the uncertainty associated with GOCI sensor. The analysis was performed for all the data and for only summer, when there is the smallest variability in the water properties. Also, the diurnal percent coefficient of variation (CV[\%]) was calculated. The root mean squared error (RMSE) from the AERONET-OC data is shown for reference.\label{tab:diurnal_var} } 
  \vspace{0.1cm}
  \centering
...
% \begin{landscape}
  \begin{table}[htbp!]
  %\internallinenumbers
  \caption{The diurnal variability was assessed with the relative difference $R\Delta_t[\%]$ with respect to the 4h time of the day value. The analysis was performed for all the data and for only summer, when there is the smallest variability in the water properties.\label{tab:rel_diff} } 
  \centering
  \includegraphics[width=19cm]{./Figures/diff_stat_combined.pdf}
  \end{table}
\end{landscape}
