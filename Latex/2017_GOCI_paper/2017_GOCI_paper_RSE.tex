...
\begin{figure}[ht]
  \centering
    \includegraphics[height=8cm]{./Figures/FilteringCriteria.pdf}
  %\internallinenumbers
...
The time series exhibit an expected seasonal cycle (\autoref{fig:GOCI_TimeSeries}-\ref{fig:GOCI_TimeSeries2}). 
...
\begin{figure}[H]
  ...
  \centering
    \includegraphics[width=15cm]{./Figures/GOCI_TimeSeries2.pdf}
    %\internallinenumbers
...
\centering
\hspace{-0.8cm}
\includegraphics[width=17cm]{./Figures/3dayseq_stats.pdf}
%\internallinenumbers
...
\centering
\hspace{-0.8cm}
\includegraphics[width=17cm]{./Figures/3dayseq_stats_par.pdf}
...
The most extreme case is the 680 nm band, where most of the values are negatives for all time of the day and solar zenith angles for both fall and winter. Similarly to $R_{rs}$, the derived biogeochemical products demonstrated a wide range of values during spring and narrower distribution during summer.

%-%-%-%-%-%-%-%-%-%-%=FIGURE=%-%-%-%-%-%-%-%-%-%-%-%-%
\begin{figure}[H]
  \centering
  \vspace{-2.5cm}
  \includegraphics[width=17cm]{./Figures/Prod_vs_zenith_season_tod.pdf}
...
GCW mean of SD diurnal values provides an approximate measure of the minimum $R_{rs}$ difference required to detect diurnal variability.\todo{Check!}
...
This mean was chosen because it reflects the value that a heritage sensor would measure with a similar acquisition time and geometry and also because this is the value that should be affected the least by solar geometry (lower solar zenith angle). 
...
The relative differences are smaller than $5\%$ for the 412, 443 and 490 nm bands for all times of the day, except for the last two times. 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=17cm]{./Figures/DiffMidThreeMean.pdf}
...
A larger number of negative values are associated with the red bands, especially for the 680 nm band, for which most of the values are negatives.
...
From these analyses, it seems that SZA does have a negatively impact in the data. 
...
Overall, the relative difference with respect to the mean of the three midday images are less than $20\%$ for all products except the $R_{rs}(680)$. 
...
The GOCI clear water region is covered by two slots of the GOCI instrument, which is visually noticeable from the images. This introduces uncertainties in the data products. A partial solution from the Korea Institute of Ocean Science and Technology (KIOST)\todo{correct?}~is underway.
