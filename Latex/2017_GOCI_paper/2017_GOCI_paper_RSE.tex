...
\begin{figure}[H]
  \hspace{-0.8cm}
  \includegraphics[width=17.5cm]{./Figures/Prod_vs_zenith_season_tod.pdf}
  \vspace{-1.3cm}
  %\internallinenumbers
...
First, a mean and standard deviation (SD) of the eight values per day was calculated for every day for the whole GOCI mission. 
Then the percent coefficient of variation (CV[\%]), defined as the SD divided by the mean, was calculated for each day.
This daily CV[\%] is an indicator of the temporal stability of the selected homogeneous ocean region throughout the day. 
As mentioned before, the hope is to have minimal variation during the day over this region. 
Then, the statistics for the daily CV[\%] were calculated: minimum, maximum, mean, median and SD (\autoref{tab:val_stats}). 
...
The mean of the diurnal CV[\%]\todo{or mean diurnal SD?}~values  for the GCW region provides an approximate measure of the minimum $R_{rs}$ difference required to detect diurnal variability.\todo{Check!}
...
% Relative difference
Second, in other to have a better estimation of the uncertainties, the relative difference of time of the day $t$ with respect to the three midday average ($RD_t$) was calculated. For each day, the mean of the three midday images ($t$ = 2, 3 and 4h) $\bar{X}_{2,3,4h}$ was calculated as
\begin{linenomath*}
\begin{equation}
...
\end{equation}
\end{linenomath*}
where $x_t$ is the satellite data at the local time $t=\text{0h,1h\dots7h}$ of the day and for this case $x_{reference}=\bar{X}_{2,3,4\text{h}}$. In order to exclude outliers, only relative differences that were within the mean plus 3 times the standard deviation were included in the analysis. The $RD_t$ is an indicator of uncertainties that are expected depending of the time of the day.

\autoref{fig:DiffMidThreeMean}\todo{change y axis in plots with $RD_t$}~and \autoref{tab:rel_diff}\todo{update table!}~show the mean and the standard deviation (error bar in the figure) of the $RD_t$ with respect to $\bar{X}_{2,3,4h}$ for each time of the day for the whole mission for $R_{rs}(\lambda)$ and Chlorophyll-{\it a}, POC and $a_g(412)$.\todo{include Difference results and filtering (at least 3 valid values to calculate mean)}
...
\begin{landscape}
  \begin{table}[htbp!]
  %\internallinenumbers
  \caption{Difference $\Delta_t$ with respect to the mean of the three midday values $\bar{X}_{2,3,4h}$.\label{tab:diff_stat} } 
  \centering
...
% \begin{landscape}
  \begin{table}[htbp!]
  %\internallinenumbers
  \caption{The diurnal variability was assessed using the diurnal percent coefficient of variation (CV[\%]) and the relative difference $RD_t$ with respect to the mean of the three midday values $\bar{X}_{2,3,4h}$.\label{tab:rel_diff} } 
  \centering
  \includegraphics[width=21cm]{./Figures/diff_stat_combined.pdf}
  \end{table}
\end{landscape}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=17cm]{./Figures/Diff4th.pdf}
    %\internallinenumbers
    \caption{Mean of the relative difference with respect to the mean of the three midday images for (a-f) $R_{rs}(\lambda)$ and (g) Chlorophyll-{\it a}, (h) POC and (i) $a_g(412)$. Error bars represent the standard deviation. Number of observations used for the statistics annotated above the error bars. \label{fig:Diff4th} } 
\end{figure}
%-%-%-%-%-%-%-%-%-%-%=END FIGURE=%-%-%-%-%-%-%-%-%-%-%-%-%
...
% limitations
\todo{Include the diurnal variability in the open ocean due to the water itself (suggested by Ivona)!}
