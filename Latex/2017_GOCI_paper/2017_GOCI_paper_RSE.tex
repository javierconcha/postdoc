...
\begin{equation}\label{eq:filteredmean}
  \text{Filtered Mean} =\frac{\sum_i(Med-1.5*\sigma) <  X_i < (Med+1.5*\sigma)}{N}
\end{equation}
where $Med$ is the unfiltered median value, $\sigma$ is standard deviation of the unfiltered data and $N$ is the number of values within $\pm 1.5*\sigma$. Note the difference with Equation 4 in \citet{Bailey2006}, where the mean of the unfiltered data was used instead of the median. The use of the median value for the calculation of the filtered mean minimize the influence of outliers.  This is the current standard approach used by the Ocean Ecology Lab at the NASA Goddard Space Flight Center.
...
\node (flag) [process, below of=MSl12, yshift=0.5cm] {Exclude flagged pixels};
\node (filtered) [process, below of=flag, yshift=-0.5cm] {Filtered Mean $=\frac{\displaystyle \sum_i^{NFP} X_i}{\displaystyle NFP}$\vspace{.3cm}\\$Med- 1.5*\sigma \geq X_i\leq Med+ 1.5*\sigma$\vspace{.3cm}\\Number Filtered Pixels (NFP)};
