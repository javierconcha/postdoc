...
%DIF DEL 2018_GOCI_vcal_20181105.tex   Tue Nov  6 12:44:55 2018
%DIF ADD 2018_GOCI_vcal.tex            Tue Nov  6 13:16:35 2018
% v1.03 - April 2017
...
% Results
Good agreement was found in vicarious gains derived using both concurrent MODISA and climatological SeaWiFS as \DIFdelbegin \DIFdel{calibration }\DIFdelend \DIFaddbegin \DIFadd{vicarious calibration data }\DIFaddend sources.  
% Conclusions
...
We recommend using the gains derived from \DIFaddbegin \DIFadd{concurrent GOCI matchups with }\DIFaddend MODISA for GOCI processing in SeaDAS/l2gen.
%
\end{abstract}
...
\section{Approach}
% vcal definition
The goal of the vicarious calibration is to minimize the difference between remotely sensed water-leaving radiance retrievals and the expected (real) water-leaving radiance. The vicarious calibration adopted here is based on the standard method used for calibrating polar-orbiting OC sensors \citep{Franz:07}. Briefly, the vicarious calibration consists in applying multiplicative correction factors or gains to the total top of the atmosphere (TOA) retrieved signal (TOA radiance signal or $L_\text{t}$) forcing the retrieved water-leaving signal (water-leaving radiance or $L_\text{w}$) to match the target (or expected) water-leaving signal (${L_\text{w}}^\text{t}$). Then, the gains for the observation sample $i^{th}$, $g_i$, are calculated as \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Franz:07}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Franz:07}}%DIFAUXCMD
}\DIFaddend :
\begin{equation}\label{eq:g_i}
  g_i={L_\text{t}}^\text{t}/L_\text{t}
\end{equation}
\DIFdelbegin \DIFdel{with
}\DIFdelend \DIFaddbegin \DIFadd{where ${L_\text{t}}^\text{t}$ is the target total TOA radiance and $L_\text{t}$ is the observed total TOA radiance (satellite data), defined as
}\DIFaddend \begin{equation}\label{eq:L_t}
  L_\text{t}=[L_\text{r}+L_\text{a}+t_{\text{d}_\text{v}}(L_\text{f}+L_\text{w})]t_{\text{g}_\text{v}}t_{\text{g}_\text{s}}f_\text{p}
\end{equation}
\DIFaddbegin \DIFadd{where $L_\text{w}$ is the water-leaving radiance, which is the quantity of interest. }\DIFaddend The terms $L_\text{r}$, $L_\text{a}$, and $L_\text{f}$ in Eq. (\ref{eq:L_t}) are the contributions from air molecules, aerosols, and white-caps or sea foam respectively. The term $t_{\text{d}_\text{v}}$ is the diffuse transmittance along the surface-sensor, and $t_{\text{g}_\text{v}}$ and $t_{\text{g}_\text{s}}$  are the transmittance accounting for the losses due to gaseous absorption in the sun-surface and surface-sensor radiant paths, respectively. The term $f_\text{p}$ is a polarization correction factor. All terms in Eq. (\ref{eq:g_i}) and Eq. (\ref{eq:L_t}) are spectrally dependent, but this dependency was omitted for brevity.

The target values could be based on {\it in situ } data, modeled data, regional climatologies or retrievals from another sensor. In practice this process is divided in two steps. First, the NIR bands are calibrated, \DIFdelbegin \DIFdel{which effectively fixes the atmospheric correction algorithm \mbox{%DIFAUXCMD
\citep{Franz:07}}%DIFAUXCMD
, }\DIFdelend and then the visible bands are calibrated using these calibrated NIR bands.  Here, both steps utilize satellite data over the same calibration site: GOCI Clear Water Subset (GCWS) (\DIFdelbegin %DIFDELCMD < \autoref{fig:GOCI_map}%%%
...
A set of vicarious gains for the GOCI's $NIR_\text{S}$ band $g_i(NIR_\text{S}$) are calculated by processing the GOCI images using the l2gen code in calibration mode with a target Angstrom coefficient of $0.9$, and over the calibration site (GCWS region). Exclusion criteria based on \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Bailey2006} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bailey2006} }%DIFAUXCMD
}\DIFaddend were applied to the $g_i$ values, with only those values that passed the exclusion criteria considered. The mission mean vicarious gain for the $NIR_\text{S}$ band $\bar{g}(745)$ (\DIFdelbegin %DIFDELCMD < \autoref{fig:GOCI_map}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig. \ref{fig:GOCI_map}}\DIFaddend , \autoref{tab:vcal_gains_comp}) was calculated as the temporal average using the mean of the semi-interquartile range (MSIQR) to minimize the effect of spurious outliers, as suggested by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Franz:07}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Franz:07}}%DIFAUXCMD
...
\begin{table}[htbp!]
%\internallinenumbers
\caption{GOCI $\bar{g}$ and standard deviations (in parentheses) calculated using the ${L_\text{wn}}^\text{t}$ from MODISA and SeaWIFS climatology. The vicarious gains derived by \DIFdelbeginFL \DIFdelFL{Wang }%DIFDELCMD < {\it %%%
\DIFdelFL{et}%DIFDELCMD < } %%%
\DIFdelFL{al. (2013) \mbox{%DIFAUXCMD
\citep{Wang:13}}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{Wang:13}}%DIFAUXCMD
}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{Ahn }%DIFDELCMD < {\it %%%
\DIFdelFL{et}%DIFDELCMD < } %%%
\DIFdelFL{al. (2015) \mbox{%DIFAUXCMD
\citep{Ahn2015}}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{Ahn2015}}%DIFAUXCMD
...
An annual daily climatology of the GCWS region from SeaWiFS observations from 1998 to 2009 (11 complete years) was generated in a similar fashion as described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Werdell:07} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Werdell:07} }%DIFAUXCMD
...
The atmospheric correction and vicarious calibration were validated using matchups from {\it in situ} observations of the \DIFdelbegin \DIFdel{AErosol RObotic NETwork-Ocean Color (}\DIFdelend AERONET-OC \DIFdelbegin \DIFdel{) }\DIFdelend \citep{Zibordi2009}. The quality-assurance (QA) level used was level 2.0, which is the highest quality for the AERONET-OC data. 
\DIFdelbegin \DIFdel{We downloaded the normalized water leaving radiance (Lwn) }\DIFdelend \DIFaddbegin \DIFadd{The normalized water-leaving radiance $L_\text{wn}$ were downloaded }\DIFaddend from the AERONET-OC website \DIFaddbegin \DIFadd{(}\url{https://aeronet.gsfc.nasa.gov/}\DIFadd{) }\DIFaddend and converted to remote sensing reflectance (\DIFdelbegin \DIFdel{Rrs}\DIFdelend \DIFaddbegin \DIFadd{$R_\text{rs}$}\DIFaddend ) by dividing \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{them }\DIFaddend by the extraterrestrial solar irradiance at the mean Earth-Sun distance (\DIFdelbegin \DIFdel{F0}\DIFdelend \DIFaddbegin \DIFadd{$F_\text{0}$}\DIFaddend ), i.e. \DIFdelbegin \DIFdel{Rrs = Lwn/ F0. F0 is derived from }\DIFdelend \DIFaddbegin \DIFadd{$R_\text{rs}=L_\text{wn}/F_\text{0}$. $F_\text{0}$ is calculated from the absolute solar irradiance }\DIFaddend spectrum derived by \cite{Thuillier2003} \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{by applying a 10 nm square band pass centered on each }{\it \DIFadd{in situ}} \DIFadd{wavelength. Also, a BRDF correction is applied as described in \mbox{%DIFAUXCMD
\cite{Bailey2006}}%DIFAUXCMD
. 
}\DIFaddend The dataset from three stations (64 matchups total) included within GOCI's footprint were used for the analysis: Gageocho (N=8; PIs: Jae-Seol Shim and Joo-Hyung Ryu), Ieodo (N=22; PIs: Young-Je Park and Hak-Yeol You), and Socheongcho (N=34, PI: Young-Je Park) (shown in Fig. \ref{fig:GOCI_map} as red circles). The AERONET-OC stations are located in different water types, which is reflected in the scatter plots in Fi{}g. \ref{fig:GOCI_AERO}, with the Ieodo site demonstrating greater $R_\text{rs}$ values than Gaeocho and Socheongcho for all bands. 
...
The selection of matchups followed the satellite validation protocols described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Bailey2006}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bailey2006}}%DIFAUXCMD
}\DIFaddend . Th{}e GOCI data were processed to L2 using the SeaDAS/l2gen package with the default atmospheric correction (aer\_opt=-2: multi-scattering with 2-band, RH-based model selection and iterative NIR correction). GOCI data acquired within a 30-minute time window of the AERONET-OC measurements were considered as potential validation matchups. A $5\times5$ GOCI pixel array centered at the {\it in situ} station location is extracted. A filtered mean is calculated from these $5\times5$ arrays in a similar manner as \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Bailey2006} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bailey2006} }%DIFAUXCMD
}\DIFaddend but applying the median to filter the data instead of the mean (i.e. \DIFdelbegin \DIFdel{$[Median-1.5*\sigma] <  X_i < [Median+1.5*\sigma]$ }\DIFdelend \DIFaddbegin \DIFadd{$[Median-1.5\times\sigma] <  X_i < [Median+1.5\times\sigma]$ }\DIFaddend with $X_i$ the value of the $i^{th}$ pixel and $\sigma$ the standard deviation). A minimum of at least half of the total pixels in the $5\times5$ array, were required to be valid (unflagged) for inclusion in the validation analysis. Additionally, a coefficient of variation (CV, filtered mean divided by the filtered standard deviation) for the visible bands 412 to 555 nm and the aerosol optical thickness (AOT) at 864 nm was calculated for each pixel array that passes the exclusion criteria described above, and then, the median value of these coefficients of variation ($\text{Median}[CV]$) was calculated. Finally, the pixel arrays whose $\text{Median}[CV]>0.15$, as suggested by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Bailey2006}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bailey2006}}%DIFAUXCMD
}\DIFaddend , are excluded from the validation analysis.

A validation analysis was performed by comparing the satellite-derive retrievals of products with the {\it in situ} observations using two performance metrics suggested by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Seegers:18}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Seegers:18}}%DIFAUXCMD
...
\begin{table}[H]
%\internallinenumbers
\caption{Statistics of the atmospheric correction and vicarious calibration validation comparing the two test approaches (in bold) with the uncalibrated and with current vicarious calibration included in SeaDAS. Results with vicarious calibration suggested by \DIFdelbeginFL \DIFdelFL{Wang }%DIFDELCMD < {\it %%%
\DIFdelFL{et}%DIFDELCMD < } %%%
\DIFdelFL{al. (2013) \mbox{%DIFAUXCMD
\citep{Wang:13} }%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
...
Overall, the \DIFaddbegin \DIFadd{monthly }\DIFaddend GOCI-derived $R_\text{rs}$ followed a similar overall trend as \DIFaddbegin \DIFadd{the monthly }\DIFaddend MODISA and VIIRS \DIFaddbegin \DIFadd{values}\DIFaddend .  Seasonally varying differences are evident, however, with the largest discrepancies during winter (Fig. \ref{fig:CrossCompAllRrs}.(a)). The ratios of $R_\text{rs}$ among all the three showed consistency in all bands, with exception to the red bands (Fig. \ref{fig:CrossCompAllRrs}.(b-d)). For the \DIFaddbegin \DIFadd{monthly }\DIFaddend GOCI/MODISA ratio, the mean value fluctuates around one for the blue-green bands and $1.6$ for the red bands, indicating that the GOCI's red band was generally brighter than MODISA. For the GOCI/VIIRS, the \DIFaddbegin \DIFadd{monthly }\DIFaddend ratio fluctuated around one for all bands with the 660 nm band having a greater spread overall. For the MODISA/VIIRS, the ratio varied close to one as well, except for the 660 nm band, which fluctuated around 0.6, suggesting that the differences observed in the red may be due to error in MODISA rather than GOCI. GOCI-derived $R_\text{rs}$ displayed a consistent behavior from year to year, with no evidence of a relative drift observed over the mission lifetime (Fig. \ref{fig:CrossCompAllRrs}.(a)). However, the sensor-to-sensor \DIFaddbegin \DIFadd{monthly }\DIFaddend ratios display a slight temporal drift \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{suggesting a slight degradation of GOCI compared to the other sensors (Fig. \ref{fig:CrossCompAllRrs}.(b,c)). }\DIFaddend A regression analysis was performed for all ratios \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{in order to quantitively assess this degradation using }\DIFaddend the slopes of \DIFaddbegin \DIFadd{this regression (}\autoref{tab:slopes}\DIFadd{). The slopes of }\DIFaddend the regression lines (dashed lines in Fig. \ref{fig:CrossCompAllRrs}.(b-d)) fluctuates from \DIFdelbegin \DIFdel{$\approx-6\times10^{-5}$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx-2.2\times10^{-2}$ per year }\DIFaddend in the blue to \DIFdelbegin \DIFdel{$\approx-1.9\times10^{-4}$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx-6.9\times10^{-2}$ per year }\DIFaddend in the red for the GOCI-MODISA and GOCI-VIIRS ratios.
%-%-%-%-%-%-%-%-%-%-%-%-%=TABLE=%-%-%-%-%-%-%-%-%-%-%-%-%-
% \begin{landscape}
\begin{table}[htbp!]
%\internallinenumbers
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[trim=30 0 25 0,clip,width=14.5cm]{./Figures/scatterRrs.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[trim=20 0 25 0,clip,width=14.5cm]{./Figures/scatterRrs.pdf}
    \DIFaddendFL %\internallinenumbers
...
The derivation of the vicarious gain for the 745-nm band was based on aerosol properties derived from MODISA over the calibration site. The derived gain is very similar to the value derived by \DIFdelbegin \DIFdel{Wang. et al. (2013) \mbox{%DIFAUXCMD
\citep{Wang:13}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Wang:13}}%DIFAUXCMD
...
\cite{Kang2010}}%DIFAUXCMD
...
 }\DIFdelend %DIF >  \cite{Kang2010}...
...
First, an aerosol model needs to be selected based on the aerosol characteristic over the calibration site. The Angstrom Coefficient expresses the dependency of the aerosol optical thickness on the wavelength of incident light \citep{Wang:2005}. Each of the 80 aerosol models described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Ahmad2010} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Ahmad2010} }%DIFAUXCMD
